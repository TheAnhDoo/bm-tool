  ...
  fastify.get('/', async (_request: FastifyRequest, reply: FastifyReply) => {
    try {
      const invites = await prisma.invite.findMany({
        ...
        include: {
          viaProfile: { select: { id: true, username: true } },
          bmProfile: { select: { id: true, username: true, bmUid: true } },
        },
      ...
      const reports = invites.map((invite) => ({
        ...
        username: invite.viaProfile?.username || null,
        idAdAccount: invite.adAccountId || null,
        idBM: invite.bmProfile ? `bm_${invite.bmProfile.id}` : null,
        bmUid: invite.bmProfile ? (invite.bmProfile as any).bmUid || null : null,
      ...
      if (format === 'csv') {
        const csv = [
          ...
          ...invites.map((invite) => {
            ...
            const username = invite.viaProfile?.username || '';
            const idAdAccount = invite.adAccountId || '';
            const idBM = invite.bmProfile ? `bm_${invite.bmProfile.id}` : '';
            const bmUid = invite.bmProfile ? (invite.bmProfile as any).bmUid || '' : '';
